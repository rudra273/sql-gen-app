...
def connect_to_db(db_credentials: Dict) -> psycopg2.extensions.connection:
    """Establish database connection using provided credentials."""
    try:
        conn = psycopg2.connect(**db_credentials)
        return conn
    except Exception as e:
        print(f"Database connection failed in schema_loader: {e}")
        raise
...
def load_db_schema(db_credentials: Dict) -> Dict:
    """
    Loads schema information from the database and returns it as a dictionary.
    """
    ...
    with open(output_file, 'w') as f:
        json.dump(schema_data, f, indent=4) # Write schema to JSON file

    print(f"Schema documentation generated in '{output_file}'")
    return schema_data # Return the schema data
